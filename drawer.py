...
class Drawer:
        ...
        if self.game_mode.game_id == game_logic.MINUTE_BEER:
            if (self.game_mode.game_message_text != ""):
                ...
                self.main_surface.blit(
                    self.text_label, (position_x, position_y))
            ...
            for i, row in enumerate(label_rows):
                ...
                position_y = self.main_surface.get_height(
                ) / 3 - i*row[0].get_height() / 2 + 1.7*i*row[0].get_height()
    ...
    def draw_player_stats(self):
        ...
        for i, p in enumerate(self.players):
            p.calculate_bac()
            stats = self.small_font.render("{}. {: <8} {:.6f} o/oo, alc consumed: {:.2f} grams, sober in {:.2f} hours".format(
                i+1, p.name, p.bac, p.alcohol_consumed, p.time_left_drunk), 1,  p.color)
            self.main_surface.blit(stats, (LINE_SPACE, self.info_bar.get_height(
            ) + LINE_SPACE + i * stats.get_height()))
