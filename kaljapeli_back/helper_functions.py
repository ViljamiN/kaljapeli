...
def Input_field(disp, ask, digits_only):
    ...
    while True:
        ...
        screen.blit(fontobject.render(ask, 1, (255, 255, 255)), (screen.get_width(
        )/2-fontobject.size(ask)[0]/2, screen.get_height()/2-75))
        screen.blit(block, rect)
        disp.blit(screen, (disp.get_width()/2-screen.get_width() /
                  2, disp.get_height()/2-screen.get_height()/2))
        pygame.display.flip()
    ...
    while True:
        ...
        for i in range(len(options)):
            ...
            block = pygame.Rect(i*block_width + 10,
                                screen.get_height()/2, block_width - 20, 75)
            if i == selection:
                pygame.draw.rect(screen, (100, 100, 255), block, 3)
            else:
                pygame.draw.rect(screen, (255, 255, 255), block, 1)
            fontobject = pygame.font.Font(None, 40)
            screen.blit(fontobject.render(options[i], 1, (255, 255, 255)), (
                i*block_width + block_width/2 - fontobject.size(options[i])[0]/2, screen.get_height()/2 + 25))

        fontobject = pygame.font.Font(None, 40)
        screen.blit(fontobject.render(question, 1, (255, 255, 255)), (screen.get_width(
        )/2-fontobject.size(question)[0]/2, screen.get_height()/2-75))
        disp.blit(screen, (disp.get_width()/2-screen.get_width() /
                  2, disp.get_height()/2-screen.get_height()/2))
        pygame.display.flip()
